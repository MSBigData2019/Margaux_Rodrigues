...
def last2(string):
    ...
    for i in range (len(string) - 2):
        if string.find(last_two,start) != -1:
            count += 1
            start += 1
    return count
...
def occurences(text):
    list_text = list(text) # PAS BIEN
    alphabet = dict()
    for i in range(len(list_text)):
        if list_text[i] in alphabet:
            alphabet[list_text[i]] += 1
        else:
            alphabet[list_text[i]] = 1
    return alphabet
...
def length_words(array):
    ...
    for i in range(len(array)):
        word_length.append(len(array[i]))
    return word_length
...
def number2digits(number):
    ...
    for i in range(len(number_as_str)):
        list_digits.append(int(number_as_str[i]))
    return list_digits
